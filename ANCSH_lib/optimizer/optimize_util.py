...
#检查矩阵是否是一个有效的旋转矩阵，通过检查矩阵与其转置的乘积与单位矩阵的差来判断
...
#实现RANSAC（随机抽样一致）算法，返回最优的拟合直线和内点集合
#关于RANSAC算法，其核心思想就是，在最小二乘法的基础上，剔除外侧一部分对拟合没有影响的直线，使得
#拟合出来的直线更优
...
"""
函数的功能是估计关节变换。它接受一个数据集dataset，一个关节类型joint_type，
和一个最佳内点集合best_inliers作为输入。
函数首先检查数据集是否满足一些基本条件，如果不满足则直接返回。然后，根据是否提供了最佳内点集合，
选择不同的样本点进行处理。
接下来，函数对源点和目标点进行预缩放和居中处理，然后计算旋转矩阵R0和R1。然后，根据关节类型，
选择不同的目标函数进行最小二乘优化。最后，函数计算平移向量，并将结果存储在一个字典中返回。
"""
#
...
#传入一个模型、数据集以及内点阈值，检验源点经模型预测的旋转、平移、尺度变换后与目标点的距离，若
#小于给定的阈值，则认为是内点，并将认为的内点的布尔集合返回
...
"""
传入参数为一组参数（包含两个旋转向量）、两组点对、关节点以及一个权重
计算经预测出的旋转矩阵旋转后，点云是否配准，并返回loss，用于目标函数优化
"""
def objective_eval_r(params, x0, y0, x1, y1, joints, isweight=True):
    ...
    #将两组点按照给定的旋转向量进行旋转，并计算目标点与旋转后的点之间的差距
...
def objective_eval_t(
    ...
    rotvec0 = params[:3].reshape((1, 3))
    rotvec1 = params[3:].reshape((1, 3))
    # 将两组点按照给定的旋转向量进行旋转，并计算目标点与旋转后的点之间的残差
    res0 = y0 - rotate_points_with_rotvec(x0, rotvec0)
    res1 = y1 - rotate_points_with_rotvec(x1, rotvec1)
    #计算两个旋转向量之间的残差
...
def rotate_pts(source, target):
    ...
    '''
    # pre-centering
    #对源点和坐标点进行预处理，即将其减去各自的均值，即将中心移动到原点附近，称为居中
    ...
    #计算行列式是否大于0，若不是，需要调整符号
    d = (np.linalg.det(U) * np.linalg.det(Vh)) < 0.0
    if d:
        D[-1] = -D[-1]
        U[:, -1] = -U[:, -1]
    #将奇异值分解输出的两个矩阵进行相乘，得到旋转矩阵
    R = np.matmul(U, Vh)
    return R

#利用罗德里格斯旋转公式计算任意一个点按照给定的旋转向量进行旋转后的点
#旋转公式见https://blog.csdn.net/qq_36162042/article/details/115488168
